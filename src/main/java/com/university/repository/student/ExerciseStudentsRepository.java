...
public interface ExerciseStudentsRepository extends JpaRepository<Exercise, UUID> {
    ...
    Optional<Exercise> findDetailById(@Param("id") UUID id);
    ...
    @Query("""
            ...
            WHERE e.lopHocPhan.id IN :lopHocPhanIds
            AND e.thoiGianKetThuc >= :now
            ORDER BY e.thoiGianKetThuc ASC
        """)
    List<Exercise> findUpcomingByLopHocPhanIds(
            @Param("lopHocPhanIds") List<UUID> lopHocPhanIds,
            @Param("now") LocalDateTime now);
