...
import com.university.enums.StudentPermission;
...
/**
 * Initializer de tao permissions mac dinh cho Student khi ung dung khoi dong.
...
public class StudentPermissionsInitializer {
    ...
    public ApplicationRunner studentPermissionsRunner(
        ...
        return args -> {
            ...
            List<Permissions> toSave = Arrays.stream(StudentPermission.values())
            ...
            // 4. Tim hoac tao role STUDENT - 1 query
            Role studentRole = roleRepository.findFirstByMaRoleIgnoreCase(STUDENT_ROLE)
                    .orElseGet(() -> {
                        Role newRole = new Role();
                        newRole.setMaRole(STUDENT_ROLE);
                        newRole.setMoTa("Hoc vien");
                        Role saved = roleRepository.save(newRole);
                        log.info("Created role: {}", STUDENT_ROLE);
                        return saved;
                    });
            ...
            Set<UUID> assignedIds = rolePermissionsRepository.findAssignedPermissionsIdsByRoleId(studentRole.getId(), permIds);

            // 7. Tao nhung RolePermissions chua co - 1 query saveAll
            List<RolePermissions> toAssign = Arrays.stream(StudentPermission.values())
                    .filter(p -> !assignedIds.contains(permMap.get(p.name()).getId()))
                    .map(p -> {
                        RolePermissions rp = new RolePermissions();
                        rp.setRole(studentRole);
            ...
            if (!toAssign.isEmpty()) {
                ...
                toAssign.forEach(rp -> log.info("Assigned permission {} to role {}",
                        rp.getPermissions().getMaPermissions(), STUDENT_ROLE));
            }

            log.info("=== Student Permissions Initialization Complete ({} permissions, {} assigned) ===",
