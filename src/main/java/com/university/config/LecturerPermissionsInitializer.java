...
/**
 * Initializer de tao permissions mac dinh cho Lecturer khi ung dung khoi dong.
...
public class LecturerPermissionsInitializer {
    ...
    public ApplicationRunner lecturerPermissionsRunner(
        ...
        return args -> {
            ...
            List<String> permissionCodes = Arrays.stream(LecturerPermission.values())
                    .map(Enum::name)
                    .toList();
            ...
            List<Permissions> toSave = Arrays.stream(LecturerPermission.values())
            ...
            // 4. Tim hoac tao role LECTURER - 1 query
            Role lecturerRole = roleRepository.findFirstByMaRoleIgnoreCase(LECTURER_ROLE)
                    .orElseGet(() -> {
                        Role newRole = new Role();
                        newRole.setMaRole(LECTURER_ROLE);
                        newRole.setMoTa("Giang vien");
            ...
            if (!toAssign.isEmpty()) {
                ...
                toAssign.forEach(rp -> log.info("Assigned permission {} to role {}",
                        rp.getPermissions().getMaPermissions(), LECTURER_ROLE));
            }

            log.info("=== Lecturer Permissions Initialization Complete ({} permissions, {} assigned) ===",
